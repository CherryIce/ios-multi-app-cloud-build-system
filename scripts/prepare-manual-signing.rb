...
def plist_json(path)
  ...
  unless status.success?
    raise SigningPreparationError, "unable to parse Xcode project #{path}: #{stderr.strip}"
  end
...
def replace_build_settings(text, configuration_id, updates)
  ...
  unless object_indexes.length == 1
    raise SigningPreparationError, "unable to locate build configuration #{configuration_id}"
  end
  ...
  unless settings_index
    raise SigningPreparationError, "build configuration #{configuration_id} has no buildSettings"
  end
  ...
  unless settings_end
    raise SigningPreparationError, "build configuration #{configuration_id} has unterminated buildSettings"
  end
  ...
  updates.each do |key, value|
    ...
    if matches.length > 1
      raise SigningPreparationError, "build configuration #{configuration_id} repeats #{key}"
    end
  ...
  expected.each do |configuration_id, settings|
    ...
    settings.each do |key, value|
      unless actual[key] == value
        raise SigningPreparationError, "failed to set #{key} for #{configuration_id}"
      end
    end
  end
...
begin
  ...
  unless options.fetch(:identity).match?(/\A[0-9A-Fa-f]{40}\z/)
    raise SigningPreparationError, "identity must be a SHA-1 fingerprint"
  end
  ...
  projects.each do |project|
    ...
    objects.each do |object_id, object|
      ...
      if object["isa"] == "PBXNativeTarget" && requested_targets.key?(target_name)
        matches[target_name] << [project, object_id, object]
      end
    end
  end
  ...
  requested_targets.each do |target_name, profile_uuid|
    ...
    unless target_matches.length == 1
      raise SigningPreparationError, "configured target #{target_name.inspect} was not found exactly once"
    end
    ...
    unless configuration_ids.length == 1
      raise SigningPreparationError,
            "target #{target_name.inspect} does not have exactly one #{configuration_name.inspect} configuration"
    end
